...
var ignoreDisabledReasons = {
    ...
    disregardFilesystemIgnores: 'Set to ignore (ignoring via the CLI is not enabled for this organisation)',
};
...
function generatePrompt(vulns, policy, prefix, options) {
    ...
    var prompts = vulns.map(function (vuln, i) {
        ...
        if (group) {
            ...
            messageIntro = fmt('✗ %s %s %s introduced %s %s', group.count, severity, issues, joiningText, group.affected.full);
            messageIntro = createSeverityBasedIssueHeading(messageIntro, vuln.severity);
        }
        else {
            infoLink += chalk.underline('/vuln/' + vuln.id);
            messageIntro = fmt('✗ %s severity %s found in %s, introduced via', severity, vuln.type === 'license' ? 'issue' : 'vuln', vulnIn, from);
            messageIntro = createSeverityBasedIssueHeading(messageIntro, vuln.severity);
        ...
        if (upgradeAvailable && (!isYarn || !isReinstall)) {
            ...
            if (vuln.parentDepType === 'extraneous') {
                ...
                reason = fmt('upgrade unavailable as %s@%s is shrinkwrapped by %s', vuln.name, vuln.version, vuln.shrinkwrap);
            }
            else if (vuln.bundled) {
                reason = fmt('upgrade unavailable as %s is bundled in vulnerable %s', vuln.bundled.slice(-1).pop(), vuln.name);
            }
            else {
